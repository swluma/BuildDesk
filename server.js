...
function normalizeIncomingMessage(message) {
  ...
  return {
    type,
    payload: message.payload ?? message.data ?? message.body ?? message,
  };
}
...
function joinRoom(socket, payload) {
  ...
  const playerId = String(payload?.player?.id || payload?.playerId || '').trim();
  const playerName = normalizePlayerName(payload?.player?.name || payload?.playerName);
...
function handleStartGame(socket, payload) {
  ...
  room.lastSyncSnapshot = payload?.syncSnapshot || null;
...
function handleGameAction(socket, payload) {
  ...
  if (action.type === 'sync_snapshot') {
    room.lastSyncSnapshot = action.payload || null;
  }
  if (
    action.type === 'end_round'
    || action.type === 'return_to_preparation'
    || (action.type === 'interrupt_match' && action.payload?.returnToPreparation)
  ) {
    room.phase = 'waiting';
    room.startedAt = null;
    broadcastRoomState(room);
...
function handleSyncRequest(socket) {
  ...
  sendSocketMessage(socket, SERVER_ROOM_EVENTS.SYNC_STATE, {
    roomCode: room.roomCode,
    room: createRoomSnapshot(room),
    snapshot: room.lastSyncSnapshot,
    syncSnapshot: room.lastSyncSnapshot,
    lastAction: room.lastAction,
    lastGameAction: room.lastAction,
  });
}
...
function serveFile(filePath, res) {
  fs.readFile(filePath, (error, data) => {
    ...
    sendResponse(res, 200, { 'Cache-Control': 'no-store', 'Content-Type': contentType }, data);
  });
}
  ...
  socket.on('message', (raw) => {
    ...
    switch (type) {
      ...
      case CLIENT_ROOM_EVENTS.LEAVE_ROOM: {
        ...
        if (room) {
          removePlayerFromRoom(room, String(payload?.playerId || payload?.player?.id || socket.playerId || ''));
        }
        break;
      }
...
server.listen(PORT, HOST, () => {
  console.log(`Block Blast Duel server running at http://${HOST}:${PORT}`);
  console.log(`Room WebSocket endpoint: ws://${HOST}:${PORT}/`);
  console.log(`Room host URL: http://${HOST}:${PORT}/?mode=host&name=Yuki&room=ABCD12&ws=ws://${HOST}:${PORT}/`);
  console.log(`Room join URL: http://${HOST}:${PORT}/?mode=join&name=Mika&room=ABCD12&ws=ws://${HOST}:${PORT}/`);
  console.log(`Local dev host URL: http://${HOST}:${PORT}/?mode=host&name=Yuki&room=ABCD12&transport=local-dev`);
});
